  ...
  if (phone) {
    ...
    handlers._tokens.verifyToken(token, phone, function (tokenIsValid) {
      if (tokenIsValid) {
        ...
        callback(403, {
          Error: 'Mssing required token in header or token is invlaid',
        });
      }
    });
  } else {
...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    ...
    _data.read('users', phone, function (err, userData) {
      if (!err && userData) {
        // HHas the sent password, compare it to the password stored in the user object
        var hashedPassword = helpers.hash(password);
        if (hashedPassword == userData.hashedPassword) {
          // If valid, create a new token with a random name. Set expiration date 1 hour in the future
          var tokenId = helpers.createRandomString(20);
          var expires = new Date() * 1000 * 60 * 60;
          var tokenObject = {
            phone,
            id: tokenId,
            expires,
...
handlers._tokens.verifyToken = function (id, phone, callback) {
  ...
  _data.read('tokens', id, function (err, tokenData) {
    if (!err && tokenData) {
      // Check that the token is for thr given user and has not expired
